...
def main() -> int:
    tasks_path = ROOT / "prompts" / "keyframe-credibility-fixes-v0.4.jsonl"
    output_dir = WORKSPACE / "HomeRun" / "06_exports" / "job-003-anime-episode-001-production" / "keyframes"
    manifest_path = ROOT / "progress" / "image-gen-web-keyframes-v0.4-credibility-fixes.jsonl"
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "v0.4").mkdir(parents=True, exist_ok=True)
    ...
    for row in rows:
        ...
        ref_arg = []
        ref_value = row.get("reference_image")
        if ref_value:
            ref = WORKSPACE / ref_value
            if not ref.exists():
                raise SystemExit(f"Missing reference image for {row['shot_id']}: {ref}")
            ref_arg = ["--reference-image", str(ref)]
